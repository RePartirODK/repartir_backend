...
public class ParrainageServices {
    ...
    public ResponseParrainage creerParrainage(int idJeune, Integer idParrain, int idFormation) {
        ...
        // On vérifie d'abord si une demande OUVERTE (sans parrain) existe déjà pour ce jeune et cette formation.
        if (parrainageRepository.findByJeune_IdAndFormation_IdAndParrainIsNull(idJeune, idFormation).isPresent()) {
            throw new EntityExistsException("Une demande de parrainage ouverte existe déjà pour cette formation.");
        }
        ...
        if (idParrain != null) {
            ...
            // Si un parrain est spécifié, on vérifie si ce parrainage spécifique existe déjà.
            if (parrainageRepository.findByJeune_IdAndFormation_IdAndParrain_Id(idJeune, idFormation, idParrain).isPresent()) {
                throw new EntityExistsException("Ce parrainage spécifique existe déjà.");
            }
        }
